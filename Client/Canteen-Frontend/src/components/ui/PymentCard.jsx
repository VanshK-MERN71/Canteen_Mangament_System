...
const PaymentCard = ({ formData, OrderData, totalAmount, setShowPayment }) => {
  ...
  useEffect(() => {
    const fetchKey = async () => {
      try {
        const response = await fetch("http://localhost:3000/api/orders/get-razorpay-key", {
          method: 'GET',
          credentials: 'include'
        });
    ...
    try {
      ...
      const options = {
        ...
        handler: async (response) => {
          ...
          try {
            const paymentResponse = await fetch("http://localhost:3000/api/orders/save-payment", {
              method: "POST",
              credentials: "include",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
    ...
    <div className="modal-overlay d-flex justify-content-center align-items-center">
      {paymentStatus === "success" ? (
        ...
        <div className="failed-message">
          ...
          <button className="btn btn-warning w-100 my-2" onClick={handlePayment}>
            Retry Payment
          </button>
        </div>
      ) : (
