...
const AdminOffers = () => {
  ...
  const fetchOffers = async () => {
    ...
    setError(null);
    try {
      const response = await fetch("http://localhost:3000/admin/offers", {
        method: "GET",
        credentials: "include",
      });
  ...
  const handleAddOffer = async () => {
    ...
    try {
      const response = await fetch("http://localhost:3000/admin/offers/add", {
        method: "POST",
        credentials: "include",
        body: formData,
      });
  ...
  const handleDeleteOffer = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/admin/offers/${id}`, {
        method: "DELETE",
  ...
  return (
    <Container className="mt-4">
      ...
      <div className="p-4 mb-4 border rounded shadow">
        <h4 className="text-secondary mb-3">➕ Add New Offer</h4>
        <Form>
          <Row className="g-3">
            <Col md={6} xs={12}>
            ...
            <tbody>
              {offers.map((offer) => (
                <tr key={offer._id}>
                  ...
                  <td>
                    <Button variant="danger" onClick={() => handleDeleteOffer(offer._id)}>
                      ❌ Delete
                    </Button>
                  </td>
                </tr>
