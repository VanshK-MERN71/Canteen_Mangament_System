...
const UsersList = () => {
  ...
  const toggleRole = async (userId, currentRole) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/users/toggle-role/${userId}`, {
        method: "PUT",
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
    ...
    <div className="container mt-4">
      ...
      {loading ? (
        ...
        <div className="table-responsive">
          <table className="table border table-hover text-center" style={{ whiteSpace: 'nowrap' }}>
              ...
              {filteredUsers.length > 0 ? (
                filteredUsers.map((user, index) => (
                  <tr key={user._id}>
                    ...
                    <td>
                      <button
                        className={`btn btn-sm ${user.role === "admin" ? "btn-outline-primary" : "btn-outline-warning"
                          } me-2`}
