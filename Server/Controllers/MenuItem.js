...
async function setItems(req, res) {
  try {
    const { name, description, price, image_url, stock_quantity, available } =
      req.body;
    ...
    if (!name || !price || !image_url) {
      return res
        ...
        .json({
          success: false,
          message: "Name, Price & Image URL are required",
        });
    }
  ...
  try {
    ...
    const cartWithDetails = cartItems.map((cartItem) => {
      const item = menuItems.find(
        (menuItem) => menuItem._id.toString() === cartItem.itemId.toString()
      );
